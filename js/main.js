	...
	firebase.database().ref('/posts').once('value').then(function(snapshot) {
		...
		$('.more-div').click(function() {
			if($('.more-button', $(this)).text() == 'More') { 
				...
				$('.image-card', $(this).parent().parent()).animate({'height': '0px'}, 500);
				$('.post-hidden-layout', $(this).parent()).show();
				$(this).parent().animate({'height': '916px'}, 500);
			}
			else {
				$('.more-button', $(this)).text('More');
				$('.image-card', $(this).parent().parent()).animate({'height': '360px'}, 500);
				$('.post-hidden-layout', $(this).parent()).hide();
				$(this).parent().animate({'height': '556px'}, 500);
			}
		});

		$(".ad-card-2").each(function() {$(this).delay(getrandomdelay()).animate({"opacity": "1"}, 700)});
		$(".ad-card-3").each(function() {$(this).delay(getrandomdelay()).animate({"opacity": "1"}, 700)});
		$(".template-div").delay(1700).animate({"opacity": "0"}, 500);
	});
});
